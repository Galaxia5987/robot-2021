...
public class TrenchShot extends SequentialCommandGroup {
    public TrenchShot(SwerveDrive swerve, Intake intake, Funnel funnel, Conveyor conveyor, Shooter shooter, VisionModule vision, PTO pto) {
        ...
        try {
            ...
            initiationToTrench = TrajectoryUtil.fromPathweaverJson(trajectoryPath);
            Path trajectoryPath2 = Filesystem.getDeployDirectory().toPath().resolve(Constants.Autonomous.TRENCH_TO_SHOOT_PATH);
            trenchToShoot = TrajectoryUtil.fromPathweaverJson(trajectoryPath2);
            Path trajectoryPath3 = Filesystem.getDeployDirectory().toPath().resolve(Constants.Autonomous.SHOOT_TO_SAFE_TRENCH_PATH);
            shootToSafeTrench = TrajectoryUtil.fromPathweaverJson(trajectoryPath3);

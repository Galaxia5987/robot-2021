...
public class SafeTrenchShot extends SequentialCommandGroup {
    public SafeTrenchShot(SwerveDrive swerve, Intake intake, Funnel funnel, Conveyor conveyor, Shooter shooter, PTO pto) {
        ...
        addCommands(new FollowPath(swerve, initiationToSafeTrench), new PickupBalls(intake, funnel, conveyor, Constants.Intake.POWER::get, true).withTimeout(2), new ToggleIntake(intake),
                new FollowPath(swerve, safeTrenchToShoot), new FeedAndShoot(pto, conveyor, shooter, Constants.Conveyor.CONVEYOR_MOTOR_POWER).withTimeout(4));
    }
}

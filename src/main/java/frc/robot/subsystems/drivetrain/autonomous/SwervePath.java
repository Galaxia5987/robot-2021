...
public class SwervePath {
    ...
    public static SwervePath loadPath(String filename, double vel, double acc) {
        PathPlannerTrajectory trajectory = PathPlanner.loadPath(filename, vel, acc);
        SwervePath path = new SwervePath();
        for (var stateVar : trajectory.getStates()) {
            var state = (PathPlannerTrajectory.PathPlannerState) stateVar;
            SwervePath.State state1 = new State(state.positionMeters, state.poseMeters.getRotation(),
                    state.velocityMetersPerSecond, state.accelerationMetersPerSecondSq, state.holonomicRotation, state.timeSeconds);
            path.states.add(state1);
        }
        return path;
    }
